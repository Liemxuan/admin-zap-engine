...
export const ContainerDevWrapper: React.FC<ContainerDevWrapperProps> = ({
    ...
    const [adjustment, setAdjustment] = useState<{ x: 'left' | 'right', y: 'top' | 'bottom', inset: boolean }>({
        x: (identity.displayName === 'ColorInspector' || identity.type?.includes('Legend')) ? 'right' : 'left',
        y: 'top',
        inset: false
    });

    React.useLayoutEffect(() => {
        if (!(isHovered || isLocked) || !labelRef.current) return;
        const rect = labelRef.current.getBoundingClientRect();
        const vw = window.innerWidth;
        const vh = window.innerHeight;

        let newX: 'left' | 'right' = adjustment.x;
        let newY: 'top' | 'bottom' = adjustment.y;
        let newInset = adjustment.inset;

        // X-axis flip
        if (rect.right > vw - 40) newX = 'right';
        else if (rect.left < 40) newX = 'left';

        // Y-axis flip (top <-> bottom)
        if (rect.top < 40) newY = 'bottom';
        else if (rect.bottom > vh - 40) newY = 'top';

        // Inset with hysteresis
        if (!adjustment.inset && rect.top < 40) newInset = true;
        else if (adjustment.inset && rect.top > 80) newInset = false;

        // Chỉ setState khi có thay đổi
        if (
            newX !== adjustment.x ||
            newY !== adjustment.y ||
            newInset !== adjustment.inset
        ) {
            setAdjustment({
                x: newX,
                y: newY,
    ...
    }, [isHovered, isLocked, adjustment.x, adjustment.y, adjustment.inset]);
    ...
    return (
        ...
        >
            ...
            <div className={`absolute inset-0 border border-dashed rounded-[inherit] pointer-events-none z-40 transition-all duration-300 ${isLocked ? 'border-indigo-400/60 bg-indigo-50/[0.05]' : (isHovered ? 'border-cyan-400 bg-cyan-50/[0.05]' : 'border-cyan-400/10 bg-transparent')}`} ></div>

            <div
                ref={labelRef}
                className={`absolute z-[9999] flex items-center cursor-pointer select-none transition-all duration-300 
                ${adjustment.y === 'top' ? 'top-0' : 'bottom-0'} ${adjustment.x === 'left' ? 'left-4' : 'right-4'}
                ${(isLocked || isHovered) ? 'opacity-100 scale-100' : 'opacity-0 scale-95'} ${(isLocked || isHovered) ? 'pointer-events-auto' : 'pointer-events-none'}
                ${adjustment.y === 'top' && !adjustment.inset ? '-translate-y-1/2' : ''} ${adjustment.y === 'bottom' && !adjustment.inset ? 'translate-y-1/2' : ''} ${adjustment.inset ? (adjustment.y === 'top' ? 'translate-y-2' : '-translate-y-2') : ''}
                `}
                onClick={handleCopy}
            >
