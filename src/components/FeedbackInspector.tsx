...
import { ProjectConfig, ThemeState, FeedbackConfig } from '../types';
...
export const FeedbackInspector: React.FC<FeedbackInspectorProps> = ({
    ...
    const updateFeedbackConfig = (section: 'toasts' | 'alerts' | 'modals', key: string, value: any) => {
        const currentFeedback: FeedbackConfig = config.generatedContent?.feedback || {
            toasts: { position: 'top-right', duration: 5000 },
            alerts: { style: 'standard' },
            modals: { overlayOpacity: 0.5, blur: false }
        };
